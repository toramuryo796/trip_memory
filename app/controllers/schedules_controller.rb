class SchedulesController < ApplicationController
  ...
  def index
    if params[:day_how_passed]
      ...
      @schedules = Schedule.where(plan_id: @plan_id, day_how_id: @day_how.passed).order("hour_id ASC")  # 日程に該当するのスケジュールを特定
      @passed = @day_how.passed
    else
      @plan = Plan.find_by(id: first_params[:plan_id])
      @plan_id = @plan.id    # plan_idを取得
      @day_how_first = DayHow.find_by(passed: 1, plan_id: @plan_id)
      @schedules = Schedule.find_by(plan_id: @plan_id, day_how_id: @day_how_first.passed)  # 日程に該当するのスケジュールを特定
